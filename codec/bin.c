...
void byte_encode(FILE *in, FILE *out, const char * opt){
	UNUSED(opt);
	int bitcount = 0;
	uint8_t byte = 0;
	uint8_t bit;
...
void byte_decode(FILE * in, FILE * out, const char * opt){
...
ENGINE_EXPORT(
	ENGINE_ENTRY("byte", 'e', byte_encode, "Convert to byte."),
	ENGINE_ENTRY("byte", 'd', byte_decode, "Convert from byte.")
);